...
import com.friend.pojo.LogInfo;
...
public class PhotoDao {
     ...
     */
    public static boolean saveFile(FileItem item,String fileName, HttpServletRequest request) {
        HttpSession session = request.getSession();
        LogInfo un = (LogInfo)session.getAttribute("user");
        String username = un.getUserName();
        String realPath = request.getSession().getServletContext().getRealPath("/");
        File savePath=new File(realPath + "\\pic\\user\\" + username);
        if(!savePath.exists()) {
            savePath.mkdirs();
        }
